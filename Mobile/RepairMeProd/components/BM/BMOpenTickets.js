...
import { View, FlatList, Text, AsyncStorage, Platform, TouchableWithoutFeedback, Dimensions, TouchableOpacity } from 'react-native';
...
class BMOpenTickets extends React.Component {
    ...
    getList() {
        authenticatedAxios({
            method: 'get',
            url: `http://${Globals.WebAPI}/api/ticket/BMOpenTicket/${this.state.assetID}?pageNumb=${this.state.pageNumb}&limit=${this.state.limit}`,
        })
            .then((response) => {
    ...
    query = () => {
        this.setState({pageNumb: 1, limit: 5}, () => {
            axios({
                method: 'get',
                url: `http://${Globals.WebAPI}/api/ticket/BMOpenTicket/search/${this.state.assetID}?pageNumb=${this.state.pageNumb}&limit=${this.state.limit}&query=${this.state.search}`,
            })
            .then((response) => {
                this.setState({list: response.data.tickets.docs});
                this.setState({totalTicks: response.data.tickets.total})
        ...
        return (
            <TouchableWithoutFeedback onPress={() => {DismissKeyboard}} accessible={false}>
                <View>
                    ...
                    />
                    <View
                    style={{position:"absolute", bottom:60, width: Dimensions.get('window').width}}
                    >
                        {isAndroid ? (
                            <TouchableOpacity
                                onPress={this._onEndReached}
                                title="LOAD MORE"
                                style={styles.greyBack}
    
                            >
                            <Text
                            style={[{textAlign:"center"},styles.smallWhiteText]}
                            >
                                LOAD MORE 
                            </Text>
                    ...
                    <FlatList
                        style={{ marginBottom: 88, paddingTop:2 }}
