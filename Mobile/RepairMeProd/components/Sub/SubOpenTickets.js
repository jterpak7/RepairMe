...
class SubOpenTickets extends React.Component {
    ...
    getList() {
        authenticatedAxios({
            method: 'get',
            url: `http://${Globals.WebAPI}/api/ticket/SubOpenTicket/${this.state.userID}?pageNumb=${this.state.pageNumb}&limit=${this.state.limit}`,
        })
        .then((response) => { 
            ...
            <Ticket
                ...
                onPress={() => {
                    this.setState({currTicket: item}, () => {
                        this.props.navigation.navigate('SubOpenTicketDetails', {ticket: this.state.currTicket});
                    });
                }}
            />
        )
    ...
    query = () => {
        this.setState({pageNumb: 1, limit: 5}, () => {
            axios({
                method: 'get',
                url: `http://${Globals.WebAPI}/api/ticket/SubOpenTicket/search/${this.state.userID}?pageNumb=${this.state.pageNumb}&limit=${this.state.limit}&query=${this.state.search}`,
            })
            .then((response) => {
                this.setState({list: response.data.tickets.docs});
                this.setState({totalTicks: response.data.tickets.total})
