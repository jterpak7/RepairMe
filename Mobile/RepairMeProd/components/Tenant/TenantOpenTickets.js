...
class TenantOpenTickets extends React.Component {
    ...
    getList() {
        authenticatedAxios({
            method: 'get',
            url: `http://${Globals.WebAPI}/api/ticket/TenantOpenTicket/${this.state.userID}?pageNumb=${this.state.pageNumb}&limit=${this.state.limit}`,
        })
        .then((response) => { 
    ...
    _listEmpty = () => {
        if(this.state.list.length === 0) {
            return <Text>Click "New Ticket" below to get started</Text>
        }
        else{
            return <Loading/>
        }
    ...
    query = () => {
        this.setState({pageNumb: 1, limit: 5}, () => {
            axios({
                method: 'get',
                url: `http://${Globals.WebAPI}/api/ticket/TenantOpenTicket/search/${this.state.userID}?pageNumb=${this.state.pageNumb}&limit=${this.state.limit}&query=${this.state.search}`,
            })
            .then((response) => {
                this.setState({list: response.data.tickets.docs});
                this.setState({totalTicks: response.data.tickets.total})
  ...
  export default TenantOpenTickets